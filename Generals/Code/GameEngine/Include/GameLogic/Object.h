...
public:
	...
	Bool getIsUndetectedDefector(void) const { return BitTestTwid(m_privateStatus, UNDETECTED_DEFECTOR); }
	void friend_setUndetectedDefector(Bool status);

	inline Bool isOffMap() const { return BitTestTwid(m_privateStatus, OFF_MAP); }

	inline Bool isCaptured() const { return BitTestTwid(m_privateStatus, CAPTURED); }
	...
	inline Bool testScriptStatusBit(ObjectScriptStatusBit b) const { return BitTestTwid(m_scriptStatus, b); }
