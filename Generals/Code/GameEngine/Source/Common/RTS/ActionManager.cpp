...
Bool ActionManager::canGetRepairedAt( const Object *obj, const Object *repairDest, CommandSourceType commandSource ) 
{
	...
	if( BitTestTwid( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			BitTestTwid( repairDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
...
Bool ActionManager::canTransferSuppliesAt( const Object *obj, const Object *transferDest ) 
{
	...
	// nothing can be done with things that are under construction
	if( BitTestTwid( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			BitTestTwid( transferDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
...
Bool ActionManager::canGetHealedAt( const Object *obj, const Object *healDest, CommandSourceType commandSource ) 
{
	...
	// nothing can be done with things that are under construction
	if( BitTestTwid( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			BitTestTwid( healDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
...
Bool ActionManager::canRepairObject( const Object *obj, const Object *objectToRepair, CommandSourceType commandSource ) 
{
	...
	// nothing can be done with things that are under construction
	if( BitTestTwid( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			BitTestTwid( objectToRepair->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
																						 ...
																						 CommandSourceType commandSource )
{
	...
	if( BitTestTwid( objectBeingConstructed->getStatusBits(), 
...
Bool ActionManager::canEnterObject( const Object *obj, const Object *objectToEnter, CommandSourceType commandSource, CanEnterType mode )
{
	...
	// nothing can be done with things that are under construction
	if( BitTestTwid( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			BitTestTwid( objectToEnter->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
	{
		return FALSE;
	}
...
//		return FALSE;
	...
	if( BitTestTwid( objectToHack->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
	{
		return FALSE;
	}
		...
		switch( spTemplate->getSpecialPowerType() )
		{
			...
			case SPECIAL_CASH_HACK:
				...
				if( target->isKindOf( KINDOF_STRUCTURE ) && r == ENEMIES )
				{
					...
					if( BitTestTwid( target->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) )
					{
						return FALSE;
					}
