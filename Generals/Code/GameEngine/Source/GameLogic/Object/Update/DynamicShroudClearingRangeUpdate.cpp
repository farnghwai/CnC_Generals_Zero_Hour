...
UpdateSleepTime DynamicShroudClearingRangeUpdate::update( void )
{
	...
	else if (m_stateCountDown > 0 && (UnsignedInt)m_stateCountDown <= m_shrinkStartDeadline  )
		m_state = DSCRU_SHRINKING;
	else if (m_stateCountDown > 0 && (UnsignedInt)m_stateCountDown <= m_sustainDeadline )
		m_state = DSCRU_SUSTAINING;
	else if (m_stateCountDown > 0 && (UnsignedInt)m_stateCountDown <= m_growStartDeadline )
