...
Object *AIPlayer::buildStructureWithDozer(const ThingTemplate *bldgPlan, BuildListInfo *info)
{
	...
	int costToBuild = bldgPlan->calcCostToBuild(m_player);
	if (costToBuild>=0 && money->countMoney() < (UnsignedInt)costToBuild) {
		return NULL;
	}
			...
			if (info->getObjectID() != INVALID_ID) {
				...
				if (bldg==NULL) {
					...
					if (bldg->getControllingPlayer() == m_player) {
						...
						if( BitTestTwid( bldg->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE) {
...
Bool AIPlayer::isPossibleToBuildTeam( TeamPrototype *proto, Bool requireIdleFactory, Bool &notEnoughMoney)
{
	...
	if (cost >= 0 && m_player->getMoney()->countMoney() < (UnsignedInt)cost)	{
		notEnoughMoney = true;
...
void AIPlayer::buildUpgrade(const AsciiString &upgrade)
{
	...
	for( const BuildListInfo *info = m_player->getBuildList(); info; info = info->getNext() )
	{
		...
		if( factory )
		{
			if( BitTestTwid( factory->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
				continue;
			if( BitTestTwid( factory->getStatusBits(), OBJECT_STATUS_SOLD ) == TRUE )
				continue;
...
void AIPlayer::checkReadyTeams( void )
{
	...
	{	// needed to scope iter.  silly ms c++.
		for ( DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamReadyQueue(); !iter.done(); iter.advance())
		{
			...
			int newFrameStart = team->m_frameStarted + 60 * LOGICFRAMES_PER_SECOND;
			Bool timeExpired = newFrameStart >=0 && (UnsignedInt)newFrameStart < TheGameLogic->getFrame();
			Bool allIdle=TRUE;	
			Bool anyIdle = FALSE;
			if (team->m_reinforcement) {
...
Bool TeamInQueue::isBuildTimeExpired()
{
	...
	int newFrame = m_frameStarted + m_team->getPrototype()->getTemplateInfo()->m_initialIdleFrames;
	if (newFrame >= 0 && TheGameLogic->getFrame() > (UnsignedInt)newFrame) {
		return true;
	}
	return false;
}
