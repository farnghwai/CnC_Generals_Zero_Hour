...
void ControlBar::populateCommand( Object *obj )
{
	...
	for( i = 0; i < MAX_COMMANDS_PER_SET; i++ )
	{
		...
		else
		{
			...
			if( BitTestTwid( commandButton->getOptions(), SCRIPT_ONLY ) )
			{
				m_commandWindows[ i ]->winHide( TRUE );
				continue;
			}
...
void ControlBar::updateContextCommand( void )
{
	...
	for( i = 0; i < MAX_COMMANDS_PER_SET; i++ )
	{
		...
		//
		if( BitTestTwid( command->getOptions(), CHECK_LIKE ))
		{

			// sanity, check like commands should have windows that are check like as well
			DEBUG_ASSERTCRASH( BitTestTwid( win->winGetStatus(), WIN_STATUS_CHECK_LIKE ),	
																												...
																												Bool forceDisabledEvaluation ) const
{
	...
	if (disabled
			&& BitTestTwid(command->getOptions(), IGNORES_UNDERPOWERED) 
	...
	if( BitTestTwid( command->getOptions(), NEED_UPGRADE ) )
	{
	...
	if( pu && pu->firstProduction() && BitTestTwid( command->getOptions(), NOT_QUEUEABLE ) )
	{
...
/// and re-target the firewall without having to stop or move in-betwen.. Thanks for reading 
		...
		{
			if( !BitTestTwid( command->getOptions(), OPTION_ONE ) )
			{
				return COMMAND_AVAILABLE;
			}
