																							...
																							GadgetGameMessage gadgetMessage )
{
	...
	//rethink it.
	if( obj && BitTestTwid( commandButton->getOptions(), SINGLE_USE_COMMAND ) )
	{
	...
	if( BitTestTwid( commandButton->getOptions(), COMMAND_OPTION_NEED_TARGET ) )
	{
	...
	else switch( commandButton->getCommandType() )
	{
		...
		case GUI_COMMAND_EVACUATE:
		{
			...
			if (BitTestTwid(commandButton->getOptions(), NEED_TARGET_POS) == FALSE) {
