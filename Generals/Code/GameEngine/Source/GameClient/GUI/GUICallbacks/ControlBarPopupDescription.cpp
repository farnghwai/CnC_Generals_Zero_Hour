...
//			m_buildToolTipLayout = NULL;
	...
	if(!cmdButton)
		return;
	if(BitTestTwid(cmdButton->winGetStyle(), GWS_PUSH_BUTTON))
	{
	...
	else
	{
		...
		if(!BitTestTwid(cmdButton->winGetStyle(), GWS_USER_WINDOW) && !BitTestTwid(cmdButton->winGetStyle(), GWS_STATIC_TEXT))
			return;
...
void ControlBar::repopulateBuildTooltipLayout( void )
{
	if(!prevWindow || !m_buildToolTipLayout)
		return;
	if(!BitTestTwid(prevWindow->winGetStyle(), GWS_PUSH_BUTTON))
		return;
