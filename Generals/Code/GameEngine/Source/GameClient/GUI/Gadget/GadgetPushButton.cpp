																						...
																						WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_MOUSE_ENTERING:
		{
			...
			if(window->winGetParent() && BitTestTwid(window->winGetParent()->winGetStyle(),GWS_HORZ_SLIDER) )
			{
		...
		case GWM_MOUSE_LEAVING:
		{
			...
			//
			if( BitTestTwid( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
		...
		case GWM_LEFT_DOWN:
		{
			...
			if( BitTestTwid( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
			{
				...
				if( BitTestTwid( instData->m_state, WIN_STATE_SELECTED ) )
					BitClear( instData->m_state, WIN_STATE_SELECTED );
				else
		...
		case GWM_LEFT_UP:
		{
			...
			//
			if( BitTestTwid( instData->getState(), WIN_STATE_SELECTED ) &&
					BitTestTwid( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
			{
		...
		case GWM_RIGHT_DOWN:
		{
			...
			if( BitTestTwid( instData->getStatus(), WIN_STATUS_RIGHT_CLICK ) )
			{
		...
		case GWM_CHAR:
		{
			switch( mData1 )
			{
				...
				case KEY_SPACE:
				{
					...
					if( BitTestTwid( mData2, KEY_STATE_UP ) )
					{
				...
				case KEY_TAB:
				{
					...
					if( BitTestTwid( mData2, KEY_STATE_DOWN ) )
...
void GadgetCheckLikeButtonSetVisualCheck( GameWindow *g, Bool checked )
{
	...
	if( BitTestTwid( g->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
	{
...
Bool GadgetCheckLikeButtonIsChecked( GameWindow *g )
{
	...
	return BitTestTwid( instData->m_state, WIN_STATE_SELECTED );
