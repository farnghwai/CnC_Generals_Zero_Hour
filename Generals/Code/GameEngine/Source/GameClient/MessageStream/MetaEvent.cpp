	...
	if (t > GameMessage::MSG_RAW_MOUSE_BEGIN && t < GameMessage::MSG_RAW_MOUSE_END )
	{
		...
		switch (t)
		{
			...
			case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_UP:
			{
				...
				int px = abs(pixelRegion.hi.x - pixelRegion.lo.x);
				int py = abs(pixelRegion.hi.y - pixelRegion.lo.y);
				if ( (px >= 0 && (UnsignedInt)px < TheMouse->m_dragTolerance) &&
					(py >= 0 && (UnsignedInt)py < TheMouse->m_dragTolerance))
				{
