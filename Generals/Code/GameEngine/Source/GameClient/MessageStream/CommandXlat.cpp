...
GameMessage::Type CommandTranslator::issueSpecialPowerCommand( const CommandButton *command, CommandEvaluateType commandType, Drawable *target, const Coord3D *pos, Object* ignoreSelObj )
{
	...
	if( BitTestTwid( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
	{
	...
	else if( BitTestTwid( command->getOptions(), NEED_TARGET_POS ) )
	{
...
GameMessage::Type CommandTranslator::issueCombatDropCommand( const CommandButton *command, CommandEvaluateType commandType, Drawable *target, const Coord3D *pos )
{
	...
	if( target != NULL && BitTestTwid( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
	{
...
GameMessage::Type CommandTranslator::issueFireWeaponCommand( const CommandButton *command, CommandEvaluateType commandType, Drawable *target, const Coord3D *pos )
{
	...
	if( BitTestTwid( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
	{
		...
		if( BitTestTwid( command->getOptions(), ATTACK_OBJECTS_POSITION ) )
		{
																														 ...
																														 CommandEvaluateType type )
{
	...
	if (obj == NULL || 
		(BitTestTwid(obj->getStatusBits(), OBJECT_STATUS_MASKED) && 
			...
			|| (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER))
	{
		...
		{
			if( obj && obj->isKindOf( KINDOF_SHRUBBERY ) && !BitTestTwid( command->getOptions(), ALLOW_SHRUBBERY_TARGET ) )
			{
			...
			if( obj && obj->isKindOf( KINDOF_MINE ) && !BitTestTwid( command->getOptions(), ALLOW_MINE_TARGET ) )
			{
			...
			if( obj && BitTestTwid( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
			{
				Relationship relationship = ThePlayerList->getLocalPlayer()->getRelationship( obj->getTeam() );
				switch( relationship )
				{
					case ALLIES:
						if( !BitTestTwid( command->getOptions(), NEED_TARGET_ALLY_OBJECT ) )
						{
							draw = NULL;
							obj = NULL;
						}	
						break;
					case ENEMIES:
						if( !BitTestTwid( command->getOptions(), NEED_TARGET_ENEMY_OBJECT ) )
						{
							draw = NULL;
							obj = NULL;
						}	
						break;
					case NEUTRAL:
						if( !BitTestTwid( command->getOptions(), NEED_TARGET_NEUTRAL_OBJECT ) )
						{
							draw = NULL;
							obj = NULL;
						}	
