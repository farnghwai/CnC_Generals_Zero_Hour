...
UnsignedInt getPickTypesForContext( Bool forceAttackMode )
{
	...
	if (command != NULL) {
		if (BitTestTwid( command->getOptions(), ALLOW_MINE_TARGET)) {
			types |= PICK_TYPE_MINES;
		}

		if (BitTestTwid( command->getOptions(), ALLOW_SHRUBBERY_TARGET ) ) {
...
//		}
		...
		// For efficiency.
		if (BitTestTwid(retVal, PICK_TYPE_MINES | PICK_TYPE_SHRUBBERY)) {
			break;
		}
	}
...
{
	if (BitTestTwid(pickTypes, PICK_TYPE_SELECTABLE)) {
		outMask.set(KINDOF_SELECTABLE);
	}

	if (BitTestTwid(pickTypes, PICK_TYPE_SHRUBBERY)) {
		outMask.set(KINDOF_SHRUBBERY);
	}

	if (BitTestTwid(pickTypes, PICK_TYPE_MINES)) {
		outMask.set(KINDOF_MINE);
	}

	if (BitTestTwid(pickTypes, PICK_TYPE_FORCEATTACKABLE)) {
