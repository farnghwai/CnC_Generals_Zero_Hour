...
void Vector4_to_Color(unsigned int *outc,const Vector4 &inc,const WW3D11Format format)
{
	// convert to ARGB 32-bit
	unsigned int color=DX11Wrapper::Convert_Color(inc);
...
void Color_to_Vector4(Vector4* outc,const unsigned int inc,const WW3D11Format format)
{
	WWASSERT(outc);
...
void Get_WW3D_Format(WW3D11Format& dest_format,WW3D11Format& src_format,unsigned& src_bpp,const Targa& targa)
{
	...
	switch (targa.Header.PixelDepth) {
		...
		case 8:
			...
			if (targa.Header.ImageType == TGA_MONO) src_format = WW3D_FORMAT_L8;
...
WW3D11Format Get_Valid_Texture_Format(WW3D11Format format, bool is_compression_allowed)
{
	int w,h,bits;
	bool windowed;

	if (!DX11Caps::Support_DXTC() || 
		...
		case WW3D_FORMAT_DXT1:
			...
			if (!DX11Caps::Support_Texture_Format(WW3D_FORMAT_DXT1) && DX11Caps::Support_Texture_Format(WW3D_FORMAT_DXT2)) {
		...
		case WW3D_FORMAT_DXT5:
			if (!DX11Caps::Support_Texture_Format(format)) format=WW3D_FORMAT_A8R8G8B8;
			break;
		}
	}
	...
	if (!DX11Caps::Support_Texture_Format(format)) {
...
unsigned Get_Bytes_Per_Pixel(WW3D11Format format)
{
	switch (format) {
	case WW3D_FORMAT_X8R8G8B8:
