...
#include "dx8caps.h"
#include "formconv.h"
#include "dx11texman.h"
#include "bound.h"
#include "dx11webbrowser.h"
...
bool								DX11Wrapper::IsInitted									= false;	
bool								DX11Wrapper::_EnableTriangleDraw						= true;

int								DX11Wrapper::CurRenderDevice							= -1;
int								DX11Wrapper::ResolutionWidth							= DEFAULT_RESOLUTION_WIDTH;
int								DX11Wrapper::ResolutionHeight							= DEFAULT_RESOLUTION_HEIGHT;
int								DX11Wrapper::BitDepth										= DEFAULT_BIT_DEPTH;
int								DX11Wrapper::TextureBitDepth							= DEFAULT_TEXTURE_BIT_DEPTH;
bool								DX11Wrapper::IsWindowed									= false;
DXGI_FORMAT					DX11Wrapper::DisplayFormat	= DXGI_FORMAT_UNKNOWN;

D3DMATRIX						DX11Wrapper::old_world;
D3DMATRIX						DX11Wrapper::old_view;
D3DMATRIX						DX11Wrapper::old_prj;

bool								DX11Wrapper::world_identity;
unsigned							DX11Wrapper::RenderStates[256];
unsigned							DX8Wrapper::TextureStageStates[MAX_TEXTURE_STAGES][32];
IDirect3DBaseTexture8 *				DX11Wrapper::Textures[MAX_TEXTURE_STAGES];
RenderStateStruct					DX11Wrapper::render_state;
unsigned							DX11Wrapper::render_state_changed;

bool								DX11Wrapper::FogEnable									= false;
D3DCOLOR							DX11Wrapper::FogColor									= 0;

IDirect3D8 *						DX11Wrapper::D3DInterface								= NULL;
IDirect3DDevice8 *					DX11Wrapper::D3DDevice									= NULL;
IDirect3DSurface8 *					DX11Wrapper::CurrentRenderTarget						= NULL;
IDirect3DSurface8 *					DX11Wrapper::DefaultRenderTarget						= NULL;

unsigned							DX11Wrapper::matrix_changes								= 0;
unsigned							DX11Wrapper::material_changes							= 0;
unsigned							DX11Wrapper::vertex_buffer_changes						= 0;
unsigned							DX11Wrapper::index_buffer_changes						= 0;
unsigned							DX11Wrapper::light_changes								= 0;
unsigned							DX11Wrapper::texture_changes							= 0;
unsigned							DX11Wrapper::render_state_changes						= 0;
unsigned							DX11Wrapper::texture_stage_state_changes				= 0;
unsigned							DX11Wrapper::_MainThreadID								= 0;
bool								DX11Wrapper::CurrentDX8LightEnables[4];

DXGI_ADAPTER_DESC		DX11Wrapper::CurrentAdapterIdentifier;
...
bool DX8Wrapper::Create_Device(void)
{
	...
	::ZeroMemory(&CurrentAdapterIdentifier, sizeof(DXGI_ADAPTER_DESC));
...
void DX8Wrapper::Enumerate_Devices()
{
	...
	for (int adapter_index=0; adapter_index<adapter_count; adapter_index++) {
		...
		::ZeroMemory(&id, sizeof(DXGI_ADAPTER_DESC));
								   ...
								   bool resize_window,bool reset_device, bool restore_assets)
{
	...
	*/
	if (IsWindowed) {
		...
		if (BitDepth==32 && D3DInterface->CheckDeviceType(0,D3DDEVTYPE_HAL,desktop_mode.Format,D3DFMT_A8R8G8B8, TRUE) == S_OK)
		...
		if (!Find_Z_Mode(DisplayFormat,_PresentParameters.BackBufferFormat,&_PresentParameters.AutoDepthStencilFormat))
		{
			...
			if (BitDepth==32) {
				...
				if (!Find_Z_Mode(_PresentParameters.BackBufferFormat,_PresentParameters.BackBufferFormat,&_PresentParameters.AutoDepthStencilFormat)) {
					_PresentParameters.AutoDepthStencilFormat=DXGI_FORMAT_UNKNOWN;
				}
			}
			else {
				_PresentParameters.AutoDepthStencilFormat=DXGI_FORMAT_UNKNOWN;
	...
	*/
	if (_PresentParameters.AutoDepthStencilFormat==DXGI_FORMAT_UNKNOWN) {
		if (BitDepth==32) {
...
bool DX8Wrapper::Find_Color_And_Z_Mode(int resx,int resy,int bitdepth,DXGI_FORMAT * set_colorbuffer,DXGI_FORMAT * set_backbuffer,DXGI_FORMAT * set_zmode)
{
	static DXGI_FORMAT _formats16[] =
	{
		D3DFMT_R5G6B5,
		D3DFMT_X1R5G5B5,
		D3DFMT_A1R5G5B5
	...
	*/
	DXGI_FORMAT * format_table = NULL;
	int format_count = 0;

	if (BitDepth == 16) {
		format_table = _formats16;
		format_count = sizeof(_formats16) / sizeof(DXGI_FORMAT);
	} else {
		format_table = _formats32;
		format_count = sizeof(_formats32) / sizeof(DXGI_FORMAT);
	}
	...
	*/
	...
	if (bitdepth==32 && *set_colorbuffer == D3DFMT_X8R8G8B8 && D3DInterface->CheckDeviceType(0,D3DDEVTYPE_HAL,*set_colorbuffer,D3DFMT_A8R8G8B8, TRUE) == S_OK)
...
// refresh rate
bool DX8Wrapper::Find_Color_Mode(DXGI_FORMAT colorbuffer, int resx, int resy, UINT *mode)
{
	UINT i,j,modemax;
	UINT rx,ry;
...
bool DX8Wrapper::Find_Z_Mode(DXGI_FORMAT colorbuffer,DXGI_FORMAT backbuffer, DXGI_FORMAT *zmode)
{
...
bool DX8Wrapper::Test_Z_Mode(DXGI_FORMAT colorbuffer,DXGI_FORMAT backbuffer, DXGI_FORMAT zmode)
{
...
void DX8Wrapper::Compute_Caps(DXGI_FORMAT display_format,DXGI_FORMAT depth_stencil_format)
{
	DX8_THREAD_ASSERT();
	DX8_Assert();
