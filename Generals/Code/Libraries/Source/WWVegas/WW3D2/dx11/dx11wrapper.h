...
#ifndef DX11_WRAPPER_H
#define DX11_WRAPPER_H
...
#define DX11_RECORD_MATRIX_CHANGE()				matrix_changes++
#define DX11_RECORD_MATERIAL_CHANGE()			material_changes++
#define DX11_RECORD_VERTEX_BUFFER_CHANGE()		vertex_buffer_changes++
#define DX11_RECORD_INDEX_BUFFER_CHANGE()		index_buffer_changes++
#define DX11_RECORD_LIGHT_CHANGE()				light_changes++
#define DX11_RECORD_TEXTURE_CHANGE()			texture_changes++
#define DX11_RECORD_RENDER_STATE_CHANGE()		render_state_changes++
#define DX11_RECORD_TEXTURE_STAGE_STATE_CHANGE() texture_stage_state_changes++

extern unsigned number_of_DX8_calls;
extern bool _DX8SingleThreaded;

void DX11_Assert();
void Log_DX11_ErrorCode(unsigned res);

WWINLINE void DX11_ErrorCode(unsigned res)
{
	if (res== S_OK) return;
	Log_DX11_ErrorCode(res);
}

#ifdef WWDEBUG
#define DX11CALL_HRES(x,res) DX8_Assert(); res = DX11Wrapper::_Get_D3D_Device11()->x; DX11_ErrorCode(res); number_of_DX8_calls++;
#define DX11CALL(x) DX8_Assert(); DX8_ErrorCode(DX11Wrapper::_Get_D3D_Device11()->x); number_of_DX8_calls++;
#define DX11CALL_D3D(x) DX11_Assert(); DX11_ErrorCode(DX11Wrapper::_Get_D3D11()->x); number_of_DX8_calls++;
#define DX11_THREAD_ASSERT() if (_DX8SingleThreaded) { WWASSERT_PRINT(DX8Wrapper::_Get_Main_Thread_ID()==ThreadClass::_Get_Current_Thread_ID(),"DX11Wrapper::DX11 calls must be called from the main thread!"); }
#else
#define DX11CALL_HRES(x,res) res = DX11Wrapper::_Get_D3D_Device11()->x; number_of_DX8_calls++;
#define DX11CALL(x) DX11Wrapper::_Get_D3D_Device11()->x; number_of_DX8_calls++;
#define DX11CALL_D3D(x) DX11Wrapper::_Get_D3D11()->x; number_of_DX8_calls++;
#define DX11_THREAD_ASSERT() ;
#endif
...
public:
#ifdef EXTENDED_STATS
	static DX11_Stats stats;
#endif
	
	static bool Init(void * hwnd);
	static void Shutdown(void);

	static void SetCleanupHook(DX11_CleanupHook *pCleanupHook) {m_pCleanupHook = pCleanupHook;};
	/*
	...
	*/
	...
	static const DXGI_ADAPTER_DESC& Get_Current_Adapter_Identifier() { return CurrentAdapterIdentifier; }
	...
	*/
	...
	static ID3D11Device* _Get_D3D_Device11() { return D3DDevice; }
...
protected:
	...
	** Internal functions
	*/
	static bool Find_Color_And_Z_Mode(int resx,int resy,int bitdepth, DXGI_FORMAT* set_colorbuffer, DXGI_FORMAT* set_backbuffer, DXGI_FORMAT* set_zmode);
	static bool Find_Color_Mode(DXGI_FORMAT colorbuffer, int resx, int resy, UINT *mode);
	static bool Find_Z_Mode(DXGI_FORMAT colorbuffer, DXGI_FORMAT backbuffer, DXGI_FORMAT*zmode);
	static bool Test_Z_Mode(DXGI_FORMAT colorbuffer, DXGI_FORMAT backbuffer, DXGI_FORMAT zmode);
	static void Compute_Caps(DXGI_FORMAT display_format, DXGI_FORMAT depth_stencil_format);

	/*
	** Protected Member Variables
	*/

	static DX11_CleanupHook *m_pCleanupHook;
	...
	static DXGI_FORMAT					DisplayFormat;
	
	static XMMATRIX						old_world;
	static XMMATRIX						old_view;
	static XMMATRIX						old_prj;
	...
	static DXGI_ADAPTER_DESC		CurrentAdapterIdentifier;

	static IDirect3D8 *					D3DInterface;			//d3d8;
	static ID3D11Device*			D3DDevice;				//d3ddevice8;	
...
WWINLINE void DX11Wrapper::_Set_DX8_Transform(D3DTRANSFORMSTATETYPE transform,const Matrix4& m)
{
	SNAPSHOT_SAY(("DX8 - SetTransform\n"));
	DX11_RECORD_MATRIX_CHANGE();
	DX11CALL(SetTransform(transform,(D3DMATRIX*)&m));
}


WWINLINE void DX11Wrapper::_Set_DX8_Transform(D3DTRANSFORMSTATETYPE transform,const Matrix3D& m)
{
	SNAPSHOT_SAY(("DX8 - SetTransform\n"));
	DX11_RECORD_MATRIX_CHANGE();
	DX11CALL(SetTransform(transform,(D3DMATRIX*)&m));
...
WWINLINE void DX11Wrapper::_Get_DX8_Transform(D3DTRANSFORMSTATETYPE transform, Matrix4& m)
{
	DX11CALL(GetTransform(transform,(D3DMATRIX*)&m));
}
...
WWINLINE void DX11Wrapper::Set_Index_Buffer_Index_Offset(unsigned offset)
{
...
WWINLINE void DX11Wrapper::Set_Fog(bool enable, const Vector3 &color, float start, float end)
{
	// Set global states
	FogEnable = enable;
...
WWINLINE void DX11Wrapper::Set_DX8_Material(const D3DMATERIAL8* mat)
{
	DX8_RECORD_MATERIAL_CHANGE();
	WWASSERT(mat);
	SNAPSHOT_SAY(("DX8 - SetMaterial\n"));
	DX11CALL(SetMaterial(mat));
}

WWINLINE void DX11Wrapper::Set_DX8_Light(int index, D3DLIGHT8* light)
{
	if (light) {
		DX11_RECORD_LIGHT_CHANGE();
		DX11CALL(SetLight(index,light));
		DX11CALL(LightEnable(index,TRUE));
		...
		DX11CALL(LightEnable(index,FALSE));
		SNAPSHOT_SAY(("DX8 - DisableLight\n"));
	}
}

WWINLINE void DX11Wrapper::Set_DX8_Render_State(D3DRENDERSTATETYPE state, unsigned value)
{
	...
	DX11CALL(SetRenderState( state, value ));
	DX8_RECORD_RENDER_STATE_CHANGE();
}

WWINLINE void DX11Wrapper::Set_DX8_Clip_Plane(DWORD Index, CONST float* pPlane)
{
	DX11CALL(SetClipPlane( Index, pPlane ));
}

WWINLINE void DX11Wrapper::Set_DX8_Texture_Stage_State(unsigned stage, D3DTEXTURESTAGESTATETYPE state, unsigned value)
{
  	if (stage >= MAX_TEXTURE_STAGES)
  	{	DX11CALL(SetTextureStageState( stage, state, value ));
  		return;
  	}
	...
	DX11CALL(SetTextureStageState( stage, state, value ));
	DX11_RECORD_TEXTURE_STAGE_STATE_CHANGE();
}

WWINLINE void DX11Wrapper::Set_DX8_Texture(unsigned int stage, IDirect3DBaseTexture8* texture)
{
  	if (stage >= MAX_TEXTURE_STAGES)
  	{	DX11CALL(SetTexture(stage, texture));
  		return;
  	}
	...
	DX11CALL(SetTexture(stage, texture));
	DX11_RECORD_TEXTURE_CHANGE();
}

WWINLINE void DX11Wrapper::_Copy_DX8_Rects(
  IDirect3DSurface8* pSourceSurface,
  CONST RECT* pSourceRectsArray,
  UINT cRects,
  IDirect3DSurface8* pDestinationSurface,
...
WWINLINE Vector4 DX11Wrapper::Convert_Color(unsigned color)
{
	Vector4 col;
...
#if 0
WWINLINE unsigned int DX11Wrapper::Convert_Color(const Vector3& color, const float alpha)
{
...
WWINLINE unsigned int DX11Wrapper::Convert_Color(const Vector4& color)
{
	WWASSERT(color.X<=1.0f);
	WWASSERT(color.Y<=1.0f);
	WWASSERT(color.Z<=1.0f);
...
WWINLINE unsigned int DX11Wrapper::Convert_Color(const Vector3& color,float alpha)
{
...
WWINLINE void DX11Wrapper::Clamp_Color(Vector4& color)
{
...
WWINLINE unsigned int DX11Wrapper::Convert_Color_Clamp(const Vector4& color)
{
	Vector4 clamped_color=color;
	DX11Wrapper::Clamp_Color(clamped_color);
...
WWINLINE void DX11Wrapper::Set_Alpha (const float alpha, unsigned int &color)
{
...
WWINLINE void DX11Wrapper::Get_Render_State(RenderStateStruct& state)
{
	state=render_state;
}

WWINLINE void DX11Wrapper::Get_Shader(ShaderClass& shader)
{
	shader=render_state.shader;
}

WWINLINE void DX11Wrapper::Set_Texture(unsigned stage,TextureClass* texture)
{
...
WWINLINE void DX11Wrapper::Set_Material(const VertexMaterialClass* material)
{
...
WWINLINE void DX11Wrapper::Set_Shader(const ShaderClass& shader)
{
...
WWINLINE void DX11Wrapper::Set_Transform(D3DTRANSFORMSTATETYPE transform,const Matrix4& m)
{
	...
	default:
		...
		DX11CALL(SetTransform(transform,(D3DMATRIX*)&m2));	
		break;
	}
}

WWINLINE void DX11Wrapper::Set_Transform(D3DTRANSFORMSTATETYPE transform,const Matrix3D& m)
{
	Matrix4 m2(m);
	switch ((int)transform) {
	case D3DTS_WORLD:
...
WWINLINE void DX11Wrapper::Set_World_Identity()
{
...
WWINLINE void DX11Wrapper::Set_View_Identity()
{
...
WWINLINE bool DX11Wrapper::Is_World_Identity()
{
	return !!(render_state_changed&(unsigned)WORLD_IDENTITY);
}

WWINLINE bool DX11Wrapper::Is_View_Identity()
{
	return !!(render_state_changed&(unsigned)VIEW_IDENTITY);
}

WWINLINE void DX11Wrapper::Get_Transform(D3DTRANSFORMSTATETYPE transform, Matrix4& m)
{
	D3DMATRIX mat;
	...
	case D3DTS_VIEW:
		...
		break;
	default:
		DX11CALL(GetTransform(transform,&mat));
		m=*(Matrix4*)&mat;
		m=m.Transpose();
		break;
	}
...
WWINLINE void DX11Wrapper::Set_Light(unsigned index, const D3DLIGHT8* light)
{
	if (light) {
...
WWINLINE const D3DLIGHT8& DX11Wrapper::Peek_Light(unsigned index)
{
	return render_state.Lights[index];;
}

WWINLINE bool DX11Wrapper::Is_Light_Enabled(unsigned index)
{
	return render_state.LightEnable[index];
}


WWINLINE void DX11Wrapper::Set_Render_State(const RenderStateStruct& state)
{
...
WWINLINE void DX11Wrapper::Release_Render_State()
{	
	if (render_state.index_buffer) {
		render_state.index_buffer->Release_Engine_Ref();
	}
