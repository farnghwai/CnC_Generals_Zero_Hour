...
static UnsignedByte * saveSurface(IDirect3DSurface9 *surface)
{
	...
	HRESULT hr=m_pDev->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, pool, &tempSurface, NULL);

	//hr=m_pDev->CopyRects(surface,NULL,0,tempSurface,NULL);
	hr=m_pDev->UpdateSurface(surface, NULL, tempSurface, NULL); // [DX9]
