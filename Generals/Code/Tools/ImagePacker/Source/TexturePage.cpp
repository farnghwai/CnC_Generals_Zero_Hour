																		 ...
																		 Int srcX, Int srcY )
{
	...
	if( srcY < imageHeight / 2 &&
			(srcY != 0 || BitTestTwid( fitBits,	ImageInfo::FIT_YBORDER_TOP )) )
	{
		...
		row = src + (buffWidth * buffBPP);
					 ...
					 (srcY != imageHeight - 1 || 
						BitTestTwid( fitBits, ImageInfo::FIT_YBORDER_BOTTOM )) )
	{
																		...
																		Bool extendAlpha )
{
	...
	if( BitTestTwid( image->m_status, ImageInfo::ROTATED90C ) )
	{
	...
	for( y = 0; y < imageHeight; y++ )
	{
		...
		for( x = 0; x < imageWidth; x++ )
		{
			...
			if( currPixel == TRUE && x == imageWidth - 1 &&
					BitTestTwid( image->m_fitBits, ImageInfo::FIT_XBORDER_RIGHT ) )
			{
			...
			if( prevPixel == FALSE && currPixel == TRUE )
			{
				...
				//
				if( x != 0 || BitTestTwid( image->m_fitBits, ImageInfo::FIT_XBORDER_LEFT ) )
				{
			...
			if( currPixel == TRUE )
			{
				...
				// top left corner
				if( x == 0 && y == 0 && 
						BitTestTwid( image->m_fitBits, ImageInfo::FIT_XBORDER_LEFT ) &&
						BitTestTwid( image->m_fitBits, ImageInfo::FIT_YBORDER_TOP ) )
				...
				// bottom right corner
								 ...
								 BitTestTwid( image->m_fitBits, ImageInfo::FIT_XBORDER_RIGHT ) &&
								 BitTestTwid( image->m_fitBits, ImageInfo::FIT_YBORDER_BOTTOM ) )
					dst = (ptr - (destWidth * destBPP)) + destBPP;
																...
																ImageInfo *image )
{
	...
	if( BitTestTwid( image->m_status, ImageInfo::ROTATED90C ) == FALSE )
	{
	...
	//
	if( BitTestTwid( TheImagePacker->getGapMethod(), 
...
Bool TexturePage::addImage( ImageInfo *image )
{
	...
	useGutter = BitTestTwid( TheImagePacker->getGapMethod(), 
											 ImagePacker::GAP_METHOD_GUTTER );
	useRGBExtend = BitTestTwid( TheImagePacker->getGapMethod(),
													ImagePacker::GAP_METHOD_EXTEND_RGB );
	...
	while( triesLeft )
	{
		...
		for( y = 0; y < m_size.y; y++ )
		{
			...
			for( x = 0; x < m_size.x; x++ )
			{
				...
				if( BitTestTwid( fitBits, ImageInfo::FIT_XBORDER_LEFT ) )
					image->m_pagePos.lo.x++;
				if( BitTestTwid( fitBits, ImageInfo::FIT_YBORDER_TOP ) )
					image->m_pagePos.lo.y++;
				if( BitTestTwid( fitBits, ImageInfo::FIT_XBORDER_RIGHT ) )
					image->m_pagePos.hi.x--;
				if( BitTestTwid( fitBits, ImageInfo::FIT_YBORDER_BOTTOM ) )
					image->m_pagePos.hi.y--;
				if( BitTestTwid( fitBits, ImageInfo::FIT_XGUTTER ) )
					image->m_pagePos.hi.x -= xGutter;
				if( BitTestTwid( fitBits, ImageInfo::FIT_YGUTTER ) )	
