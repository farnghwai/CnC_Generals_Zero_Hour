...
Int W3DProjectedShadowManager::renderProjectedTerrainShadow(W3DProjectedShadow *shadow, AABoxClass &box)
{
	...
	if (TheTerrainRenderObject)
	{
		...
		{	//flush the buffer by drawing the contents and re-locking again
			...
			if (shadowVertexBufferD3D->Lock(0, numVerts * sizeof(SHADOW_VOLUME_VERTEX), reinterpret_cast<void**>(&pvVertices), D3DLOCK_DISCARD) != D3D_OK)
				return 0;
			nShadowVertsInBuf=0;
		...
		{	
			...
			if (shadowVertexBufferD3D->Lock(nShadowVertsInBuf * sizeof(SHADOW_VOLUME_VERTEX), numVerts * sizeof(SHADOW_VOLUME_VERTEX), reinterpret_cast<void**>(&pvVertices), D3DLOCK_NOOVERWRITE) != D3D_OK)
				return 0;
		}
...
*/
		...
		{	//flush the buffer by drawing the contents and re-locking again
			...
			if (shadowDecalVertexBufferD3D->Lock(0, numVerts * sizeof(SHADOW_DECAL_VERTEX), reinterpret_cast<void**>(&pvVertices), D3DLOCK_DISCARD) != D3D_OK)
				return;
		...
		{			
			...
			if (shadowDecalVertexBufferD3D->Lock(nShadowDecalVertsInBuf * sizeof(SHADOW_DECAL_VERTEX), numVerts * sizeof(SHADOW_DECAL_VERTEX), reinterpret_cast<void**>(&pvVertices), D3DLOCK_NOOVERWRITE) != D3D_OK)
				return;
		}
...
*/
		...
		{	//flush the buffer by drawing the contents and re-locking again
			...
			if (shadowDecalIndexBufferD3D->Lock(0, numIndex * sizeof(short), reinterpret_cast<void**>(&pvIndices), D3DLOCK_DISCARD) != D3D_OK)
				return;
		...
		{			
			...
			if (shadowDecalIndexBufferD3D->Lock(nShadowDecalIndicesInBuf*sizeof(short),numIndex*sizeof(short), reinterpret_cast<void**>(&pvIndices),D3DLOCK_NOOVERWRITE) != D3D_OK)
				return;
		}
