...
static void drawButtonText( GameWindow *window, WinInstanceData *instData )
{
	...
	text->setWordWrapCentered( BitTestTwid( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
	text->setWordWrap(size.x);
	// get the right text color
	if( BitTestTwid( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		textColor = window->winGetDisabledTextColor();
		dropColor = window->winGetDisabledTextBorderColor();
	}  // end if, disabled
	else if( BitTestTwid( instData->getState(), WIN_STATE_HILITED ) )
	{
...
void W3DGadgetPushButtonDraw( GameWindow *window, WinInstanceData *instData )
{
	...
	if( BitTestTwid( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		...
		if( BitTestTwid( instData->getState(), WIN_STATE_SELECTED ) )
		{
																	 ...
																	 WinInstanceData *instData )
{
	...
	{
		if( BitTestTwid( instData->getState(), WIN_STATUS_USE_OVERLAY_STATES ) )
		{
			ICoord2D size, start;
			// get window position
																	 ...
																	 WinInstanceData *instData )
{
	...
	if( !BitTestTwid( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
	...
	if( image )
	{
		...
		{	
			...
			{
				if( !BitTestTwid( window->winGetStatus(), WIN_STATUS_NOT_READY ) )
				{
					...
					if( !BitTestTwid( window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR ) )
					{
	...
	if( BitTestTwid( window->winGetStatus(), WIN_STATUS_FLASHING ) )
	{
	...
	if( BitTestTwid( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
		...
		if( pushedOverlayIcon && hilitedOverlayIcon )
		{
			if(BitTestTwid(window->winGetStatus(), WIN_STATUS_ENABLED))
			{
				if (BitTestTwid( instData->getState(), WIN_STATE_HILITED ))
				{
					if( BitTestTwid( instData->getState(), WIN_STATE_SELECTED ) )
					{
  			...
  			else if( BitTestTwid( instData->getState(), WIN_STATE_SELECTED ) )
 				{
